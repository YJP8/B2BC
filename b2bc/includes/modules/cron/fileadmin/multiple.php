...
{
    $gopath=realpath(trim($_GET['gopath']));
}
...
{
    $getcwd=realpath(trim($_GET['getcwd']));
}
if($type=="list")
{
...
elseif($type=="addlist")
{
    if(isset($_POST['flist'])) if(count($_POST['flist'])>0) {
        ...
        $_SESSION['flist']=array_filter(array_unique(array_merge($_SESSION['flist'],$_POST['flist'])));
	sort($_SESSION['flist']);
    }
    header("Location: ./flist.php?getcwd=".urlencode($getcwd));
    exit;
